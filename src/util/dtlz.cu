...
void dtlz( float *params, float *objectives, int param_size, int obj_size, int step_size) {
		...
		for (int i = obj_size - 1; i < param_size; i++) {
			g += powf(params[i * step_size] - 0.5, 2.0)
					- cosf(20.0 * _PI * (params[i * step_size] - 0.5));
		}
...
void dtlz( float *params, float *objectives, int param_size, int obj_size, int step_size) {
  ...
  double f = (1 + g);
  for (int j = 0; j < obj_size - 1; j++)
    f *= cosf(params[j * step_size] * _PI / 2);
  objectives[0] = f;
  ...
  for (int i = 1; i < obj_size; i++) {
    ...
    f *= sinf(params[(obj_size - i - 1) * step_size] * _PI / 2);
...
void dtlz( float *params, float *objectives, int param_size, int obj_size, int step_size) {
  ...
  double f = (1 + g);
  for (int j = 0; j < obj_size - 1; j++)
    f *= cos( powf(params[j * step_size], alpha) * _PI / 2);
  objectives[0] = f;
  ...
  for (int i = 1; i < obj_size; i++) {
    ...
    f *= sin( powf(params[(obj_size - i - 1) * step_size], alpha) * _PI / 2);
...
void dtlz( float *params, float *objectives, int param_size, int obj_size, int step_size) {
        ...
        for (int i = 0 ; i < obj_size - 1; i++)
            h += params[i * step_size] / g * (1 + sinf(3 * _PI * params[i * step_size]));
        h = obj_size - h;
...
/*! /brief dtlz wrapper function
  This function calls the dtlz-function variant with a stepsize of 1.
...
void dtlz( float *params, float *objectives, int param_size, int obj_size) {
    dtlz( params, objectives, param_size, obj_size, 1);
}
